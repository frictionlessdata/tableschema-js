...
describe('Models', () => {
  ...
  it('have a correct number of header required columns', done => {
    (new Schema(SCHEMA, true)).then(schema => {
      const headers = schema.getRequiredHeaders()
  ...
  it('fields are not required by default', done => {
    ...
    model.then(schema => {
      assert.isArray(schema.getRequiredHeaders())
      assert.equal(schema.getRequiredHeaders().length, 1)
      done()
    }, (error) => {
      assert.isNull(error)
      done()
  ...
  it('should load json file', done => {
    ...
    model.then((schema) => {
      assert.equal(schema.headers().length, 5)
      assert.equal(schema.getRequiredHeaders().length, 2)
      assert.isTrue(schema.hasField('id'))
      assert.isTrue(schema.hasField('height'))
      assert.isTrue(schema.hasField('age'))
      assert.isTrue(schema.hasField('name'))
  ...
  it('should return true on test', done => {
    (new Schema(SCHEMA)).then(schema => {
      assert.isTrue(schema.test('height', 1))
      done()
    }, (error) => {
      assert.isNull(error)
      done()
  ...
  it('convert row', done => {
    (new Schema(SCHEMA)).then(schema => {
      const value = ['string', '10.0', '1', 'string', 'string']
        , convertedRow = schema.convertRow(value)
  ...
  it('shouldn\'t convert row with wrong type multiple errors', done => {
    (new Schema(SCHEMA)).then(schema => {
      try {
        schema.convertRow(['string', 'notdecimal', '10.6', 'string', true])
      } catch (e) {
        assert.isArray(e)
