...
describe('Constraints', () => {
  ...
  it('require throws error if value is "null"', (done) => {
    for (const value of utilities.NULL_VALUES) {
      assert.throws(() => {
        constraints.required('name', value, true)
      }, Error)
    }
    done()
  })
  ...
  it('minimum throws error if value type is not supported', (done) => {
    assert.throws(() => {
      constraints.minimum('name', 'string', 5)
    }, Error)
    done()
  })

  it('minimum return true if value bigger than minimum', (done) => {
    assert.isTrue(constraints.minimum('name', '4', 4))
    assert.isTrue(constraints.minimum('name', '4', 3))
    assert.isTrue(
      constraints.minimum('name', moment('2016-06-26 00:00:01'), '2016-06-26'))
    done()
  })

  it('maximum throws error if value bigger than maximum', (done) => {
    assert.throws(() => {
      constraints.maximum('name', '9.01', 5)
    }, Error)
    assert.throws(() => {
      constraints.maximum('name', moment('2016-08-26'), '2016-06-26')
    }, Error)
    done()
  })
  ...
  it('maximum throws error if value type is not supported', (done) => {
    assert.throws(() => {
      constraints.maximum('name', 'string', 5)
    }, Error)
    done()
  })

  it('maximum return true if value less than maximum', (done) => {
    assert.isTrue(constraints.maximum('name', '4', 4))
    assert.isTrue(constraints.maximum('name', 4.3, 8))
    assert.isTrue(
      constraints.maximum('name', moment('2016-06-25 00:11:12'), '2016-06-26'))
    done()
  })

  it('pattern throws error if value is not match', (done) => {
    assert.throws(() => {
      constraints.pattern('name', 'String without match', '/test/gim')
    }, Error)
    done()
  })

  it('pattern return true if value match', (done) => {
    assert.isTrue(constraints.pattern('name', 'String for test', '/test/gim'))
    done()
  })
})
