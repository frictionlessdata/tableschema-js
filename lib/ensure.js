...
function ensure(schema) {
  ...
  if (!_.every(schema.fields, function (field) {
    return utilities.isHash(field);
  })) {
    valid = false;
  ...
  if (!_.every(schema.fields, function (field) {
    return !field.constraints || utilities.isHash(field.constraints);
  })) {
    valid = false;
  ...
  _.chain(schema.fields).filter(function (field) {
    return Boolean(field.constraints);
  }).each(function (field) {
    // IF `required` key, then it is a boolean
    if (field.constraints.required && !_.isBoolean(field.constraints.required)) {
      valid = false;
      addError('required constraint must be a boolean.');
    }

    // IF `minLength` key, then it is an integer
    if (field.constraints.minLength && !_.isNumber(field.constraints.minLength)) {
      valid = false;
      addError('minLength constraint must be an integer.');
    }

    // IF `maxLength` key, then it is an integer
    if (field.constraints.maxLength && !_.isNumber(field.constraints.maxLength)) {
      valid = false;
  ...
  if (schema.foreignKeys) {
    ...
    if (!_.every(schema.foreignKeys, function (FK) {
      return utilities.isHash(FK);
    })) {
      valid = false;
    ...
    if (!_.every(schema.foreignKeys, function (FK) {
      return _.isString(FK.fields) || _.isArray(FK.fields);
    })) {
      valid = false;
