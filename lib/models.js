...
SchemaModel.prototype = {
  ...
  cast: function cast(fieldName, value, index) {
   ...
   */
  ,
  getConstraints: function getConstraints(fieldName) {
  ...
  ,
  getFieldsByType: function getFieldsByType(typeName) {
    return _underscore._.where(this.fields(), { type: typeName });
  }

  // Return the `type` for `fieldName`.
  ,
  getType: function getType(fieldName, index) {
  ...
  requiredHeaders: function requiredHeaders() {
  ...
  validateAndExpand: function validateAndExpand(value) {
    if (_underscore._.isUndefined(value) || _underscore._.isNull(value)) {
      throw new Error('Invalid JSON');
    }

    if (!(0, _ensure.ensure)(value)[0]) {
